...
// network. It exposes balance and last-tx-reference queries against L0,
...
// Client represents a DAG network client.
type Client struct {
...
// network. Returns ErrInvalidAddress for empty input, ErrNotFound if the
...
func (c *Client) Balance(ctx context.Context, address string) (tx.Amount, error) {
	...
	url := c.l0URL + "/dag/" + address + "/balance"
